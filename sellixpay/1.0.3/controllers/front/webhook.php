        ...
        try {
            ...
            if ($order_id > 0) {
                ...
                if ($sellix_order['status'] == 'PROCESSING') {
                    ...
                    $comment = sprintf($this->module->l('Sellix payment processing. '));
                    ...
                    $comment = sprintf($this->module->l('Payment is pending. '));
                    $comment .= sprintf($this->module->l('Transaction ID: ') . $sellix_order['uniqid']);
                    $comment .= sprintf($this->module->l(' Status: ') . $sellix_order['status']);

                    $new_history = new OrderHistory();
                    $new_history->id_order = (int) $order_id;
                    $new_history->changeIdOrderState(Configuration::get('SELLIXPAY_WAITING_PAYMENT_STATUS'), $order_id, true);
                    $new_history->add();
                ...
                $transaction_id = $sellix_order['uniqid'];
                $this->module->log('Sellixpay: Cart #' . $cart->id . ' (' . $transaction_id . '). Status: ' . $sellix_order['status']);
                ...
                if ($sellix_order['status'] == 'PROCESSING') {
                    ...
                    $order_status = Configuration::get('SELLIXPAY_HOLD_PAYMENT_STATUS');
                    $comment = sprintf($this->module->l('Awaiting crypto currency confirmations. '));
                    $comment .= sprintf($this->module->l('Transaction ID: ') . $sellix_order['uniqid']);
                    $comment .= sprintf($this->module->l(' Status: ') . $sellix_order['status']);
                } elseif ($sellix_order['status'] == 'PARTIAL') {
